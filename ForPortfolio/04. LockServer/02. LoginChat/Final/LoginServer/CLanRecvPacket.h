...
1. CLanRecvPacket::Alloc으로 할당, 데이터 삽입 전 Clear로 초기화.
...
{
	friend CObjectPool<CLanRecvPacket>;
	friend CLanMsg;

public:
	static CLanRecvPacket* Alloc();
	static bool Free(CLanRecvPacket* packet);
	...
	stack<CLanMsg*> _msgs;
	SRWLOCK _lock;
	static CObjectPool<CLanRecvPacket> _pool;

...
{
	en_MONITOR_DATA_TYPE_LOGIN_SERVER_RUN = 1,			// 로그인서버 실행여부 ON / OFF
	en_MONITOR_DATA_TYPE_LOGIN_SERVER_CPU = 2,			// 로그인서버 CPU 사용률
	en_MONITOR_DATA_TYPE_LOGIN_SERVER_MEM = 3,			// 로그인서버 메모리 사용 MByte
	en_MONITOR_DATA_TYPE_LOGIN_SESSION = 4,				// 로그인서버 세션 수 (컨넥션 수)
	en_MONITOR_DATA_TYPE_LOGIN_AUTH_TPS = 5,			// 로그인서버 인증 처리 초당 횟수
	en_MONITOR_DATA_TYPE_LOGIN_PACKET_POOL = 6,			// 로그인서버 패킷풀 사용량

	en_MONITOR_DATA_TYPE_GAME_SERVER_RUN = 10,			// GameServer 실행 여부 ON / OFF
	en_MONITOR_DATA_TYPE_GAME_SERVER_CPU = 11,			// GameServer CPU 사용률
	en_MONITOR_DATA_TYPE_GAME_SERVER_MEM = 12,			// GameServer 메모리 사용 MByte
	en_MONITOR_DATA_TYPE_GAME_SESSION = 13,				// 게임서버 세션 수 (컨넥션 수)
	en_MONITOR_DATA_TYPE_GAME_AUTH_PLAYER = 14,			// 게임서버 AUTH MODE 플레이어 수
	en_MONITOR_DATA_TYPE_GAME_GAME_PLAYER = 15,			// 게임서버 GAME MODE 플레이어 수
	en_MONITOR_DATA_TYPE_GAME_ACCEPT_TPS = 16,			// 게임서버 Accept 처리 초당 횟수
	en_MONITOR_DATA_TYPE_GAME_PACKET_RECV_TPS = 17,		// 게임서버 패킷처리 초당 횟수
	en_MONITOR_DATA_TYPE_GAME_PACKET_SEND_TPS = 18,		// 게임서버 패킷 보내기 초당 완료 횟수
	en_MONITOR_DATA_TYPE_GAME_DB_WRITE_TPS = 19,		// 게임서버 DB 저장 메시지 초당 처리 횟수
	en_MONITOR_DATA_TYPE_GAME_DB_WRITE_MSG = 20,		// 게임서버 DB 저장 메시지 큐 개수 (남은 수)
	en_MONITOR_DATA_TYPE_GAME_AUTH_THREAD_FPS = 21,		// 게임서버 AUTH 스레드 초당 프레임 수 (루프 수)
	en_MONITOR_DATA_TYPE_GAME_GAME_THREAD_FPS = 22,		// 게임서버 GAME 스레드 초당 프레임 수 (루프 수)
	en_MONITOR_DATA_TYPE_GAME_PACKET_POOL = 23,			// 게임서버 패킷풀 사용량
	...
	en_MONITOR_DATA_TYPE_CHAT_SESSION = 33,				// 채팅서버 세션 수 (컨넥션 수)
	...
	en_MONITOR_DATA_TYPE_MONITOR_AVAILABLE_MEMORY = 44,		// 서버컴퓨터 사용가능 메모리 MByte
};


enum en_PACKET_CS_MONITOR_TOOL_RES_LOGIN
{
	en_MONITOR_TOOL_LOGIN_OK = 1,				// 로그인 성공
	en_MONITOR_TOOL_LOGIN_ERR_NOSERVER = 2,		// 서버이름 오류 (매칭미스)
	en_MONITOR_TOOL_LOGIN_ERR_SESSIONKEY = 3,	// 로그인 세션키 오류
};

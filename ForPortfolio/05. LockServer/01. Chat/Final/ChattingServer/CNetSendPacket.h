...
1. CNetSendPacket::Alloc으로 할당, 데이터 삽입 전 Clear로 초기화.
...
{
	friend CObjectPool<CNetSendPacket>;
	friend CNetMsg;

public:
	static CObjectPool<CNetSendPacket> _pool;

	inline static CNetSendPacket* Alloc()
	{
		CNetSendPacket* packet = _pool.Alloc();
		packet->Clear();
		return packet;
	}

	inline static bool Free(CNetSendPacket* packet)
	{

...
unsigned int __stdcall CLanServer::AcceptThread(void* arg)
{
	...
	for (;;)
	{
		...
		CLanSession* pSession = pNetServer->_sessions[idx];
...
unsigned int __stdcall CLanServer::ReleaseThread(void* arg)
{
	...
	for (;;)
	{
		...
		{
			unsigned __int64 ID = pNetServer->_pReleaseQ->Dequeue();
			unsigned __int64 idx = ID & pNetServer->_indexMask;
			idx >>= __ID_BIT__;
			CLanSession* pSession = pNetServer->_sessions[idx];
			if (pSession == nullptr) continue;

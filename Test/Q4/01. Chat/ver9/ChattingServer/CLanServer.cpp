...
bool CLanServer::SendPacket(unsigned __int64 sessionID, CLanPacket* packet)
{
	...
	pSession->_sendBuf.Enqueue((char*)&packet, sizeof(CLanPacket*));
...
bool CLanServer::HandleSendCP(CLanSession* pSession, int sendBytes)
{
	for (int i = 0; i < pSession->_sendCount; i++)
	{
		...
		CLanPacket* packet;
		pSession->_saveBuf.Dequeue((char*)&packet, sizeof(CLanPacket*));
...
bool CLanServer::SendPost(CLanSession* pSession)
{
	...
	while (pSession->_sendBuf.GetUseSize() > 0)
	{
		...
		pSession->_sendBuf.Dequeue((char*)&packet, sizeof(CLanPacket*));

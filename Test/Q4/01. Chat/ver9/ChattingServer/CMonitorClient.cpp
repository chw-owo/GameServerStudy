	...
	while (pMonitor->_serverAlive)
	{
		...
		*packet << (int)0;
		...
		while (pMonitor->_serverAlive && pMonitor->_connected) 
		{
			...
			long chatTime = InterlockedExchange(&pServer->_chatSum, 0);
			long netUpdate1 = InterlockedExchange(&pServer->_NetUpdates[0], 0);
			long netUpdate2 = InterlockedExchange(&pServer->_NetUpdates[1], 0);
			long netTime1 = InterlockedExchange(&pServer->_netSums[0], 0);
			long netTime2 = InterlockedExchange(&pServer->_netSums[1], 0);			
			long netUpdate = netUpdate1 + netUpdate2;
			long netTime = netTime1 + netTime2;

			long SendPost1 = InterlockedExchange(&pServer->_SendPost[0], 0);
			long SendPost2 = InterlockedExchange(&pServer->_SendPost[1], 0);
			long SendComp1 = InterlockedExchange(&pServer->_SendComp[0], 0);
			long SendComp2 = InterlockedExchange(&pServer->_SendComp[1], 0);
			long RecvComp1 = InterlockedExchange(&pServer->_RecvComp[0], 0);
			long RecvComp2 = InterlockedExchange(&pServer->_RecvComp[1], 0);
			long SessionNull1 = InterlockedExchange(&pServer->_SessionNull[0], 0);
			long SessionNull2 = InterlockedExchange(&pServer->_SessionNull[1], 0);
			long SessionDead1 = InterlockedExchange(&pServer->_SessionDead[0], 0);
			long SessionDead2 = InterlockedExchange(&pServer->_SessionDead[1], 0);
			long SendPost = SendPost1 + SendPost2;
			long SendComp = SendComp1 + SendComp2;
			long RecvComp = RecvComp1 + RecvComp2;
			long SessionNull = SessionNull1 + SessionNull2;
			long SessionDead = SessionDead1 + SessionDead2;


			long processOn = pServer->_mm->GetProcessOnOff();
			float processCPU = pServer->_mm->GetProcessCPUTime();
			float processUserCPU = pServer->_mm->GetProcessUserCPUTime();
			float processKernelCPU = pServer->_mm->GetProcessKernelCPUTime();
			...
			swprintf_s(text, dfMONITOR_TEXT_LEN,
				...
				processCPU, processUserCPU, processKernelCPU, processMem,
				updateTPS, chatTime, netUpdate, netTime,
				SendPost, SendComp, RecvComp, SessionNull, SessionDead,

...
bool CLanClient::SendPacket(CLanPacket* packet)
{
	...
	_client->_sendBuf.Enqueue((char*)&packet, sizeof(CLanPacket*));
...
bool CLanClient::HandleSendCP(int sendBytes)
{
	...
	{
		if (_client->_saveBuf.GetUseSize() == 0) break;
		CLanPacket* packet = nullptr;
		_client->_saveBuf.Dequeue((char*)&packet, sizeof(CLanPacket*));
...
{
	int cnt = 0;
	while(_client->_sendBuf.GetUseSize() > 0)
	{
		if (cnt == dfWSASENDBUF_CNT) break;
		CLanPacket* packet = nullptr;
		_client->_sendBuf.Dequeue((char*)&packet, sizeof(CLanPacket*));
		_client->_wsaSendbuf[cnt].buf = packet->GetLanPacketReadPtr();
		_client->_wsaSendbuf[cnt].len = packet->GetLanPacketSize();
		_client->_sendBuf.Enqueue((char*)&packet, sizeof(CLanPacket*));
	}
	_client->_sendCount = cnt;
	...
	int sendRet = WSASend(_client->_sock, _client->_wsaSendbuf,
		cnt, &sendBytes, 0, (LPOVERLAPPED)&_client->_sendComplOvl, NULL);

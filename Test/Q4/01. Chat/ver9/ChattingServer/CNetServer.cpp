...
bool CNetServer::SendPacket(unsigned __int64 sessionID, CNetPacket* packet)
{
	...
	pSession->_sendBuf.Enqueue((char*)&packet, sizeof(CNetPacket*));
...
unsigned int __stdcall CNetServer::NetworkThread(void* arg)
{
	...
	for (;;)
	{
		...
		pNetServer->_NetUpdates[idx]++;
		pNetServer->_netStarts[idx] = timeGetTime();

		if (pNetServer->_networkAlive == 1) 
		{
			pNetServer->_netSums[idx] += (timeGetTime() - pNetServer->_netStarts[idx]);
			break;
		}
...
bool CNetServer::HandleSendCP(CNetSession* pSession, int sendBytes)
{
	...
	{
		if (pSession->_saveBuf.GetUseSize() == 0) break;
		CNetPacket* packet = nullptr;
		pSession->_saveBuf.Dequeue((char*)&packet, sizeof(CNetPacket*));
...
bool CNetServer::SendPost(CNetSession* pSession)
{
	...
	while (pSession->_sendBuf.GetUseSize() > 0)
	{
		if (cnt == dfWSASENDBUF_CNT) break;
		CNetPacket* packet = nullptr;
		pSession->_sendBuf.Dequeue((char*)&packet, sizeof(CNetPacket*));

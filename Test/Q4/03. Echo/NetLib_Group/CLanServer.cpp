...
bool CLanServer::NetworkTerminate()
{
	...
	{
		wchar_t stErrMsg[dfMSG_MAX];
		swprintf_s(stErrMsg, dfMSG_MAX, L"%s[%d]: LanServer Already Terminate", _T(__FUNCTION__), __LINE__);
...
unsigned int __stdcall CLanServer::NetworkThread(void* arg)
{
	...
	for (;;)
	{
		...
		CSession* pSession = pLanServer->AcquireSessionUsage(sessionID);
...
bool CLanServer::SendPacket(unsigned __int64 sessionID, CPacket* packet, bool disconnect)
{
	CSession* pSession = AcquireSessionUsage(sessionID);
	if (pSession == nullptr) return false;
...
void CLanServer::HandleRecvCP(CSession* pSession, int recvBytes)
{
...
void CLanServer::HandleSendCP(CSession* pSession, int sendBytes)
{
	int i = 0;

...
1. CNetSendPacket::Alloc()로 할당, 데이터 삽입 전 Clear()로 초기화.
...
class CNetGroup;
class CNetSendPacket
{
	friend CTlsPool<CNetSendPacket>;

public:
	static CNetSendPacket* Alloc()
	{
		CNetSendPacket* packet = CNetSendPacket::_pool.Alloc();
		packet->Clear();
		return packet;
	}

	static bool Free(CNetSendPacket* packet)
	{
		if (InterlockedDecrement(&packet->_usageCount) == 0)
		{
			CNetSendPacket::_pool.Free(packet);
			return true;
		}
		return false;
	}
	...
	static CTlsPool<CNetSendPacket> _pool;
...
protected:
	inline CNetSendPacket()
		: _iBufferSize(dfPACKET_DEF_SIZE), _iPayloadSize(0), _iHeaderSize(dfNETHEADER_LEN),
	...
	inline CNetSendPacket(int iBufferSize)
...
public:
	inline CNetSendPacket& operator = (CNetSendPacket& clSrCNetSendPacket)
	{
		*this = clSrCNetSendPacket;
		return *this;
	}

	inline CNetSendPacket& operator << (float fValue)
	{
	...
	inline CNetSendPacket& operator << (double dValue)
	{
	...
	inline CNetSendPacket& operator << (char chValue)
	{
	...
	inline CNetSendPacket& operator << (unsigned char byValue)
	{
	...
	inline CNetSendPacket& operator << (short shValue)
	{
	...
	inline CNetSendPacket& operator << (unsigned short wValue)
	{
	...
	inline CNetSendPacket& operator << (int iValue)
	{
	...
	inline CNetSendPacket& operator << (long lValue)
	{
	...
	inline CNetSendPacket& operator << (__int64 iValue)
	{
		if (_iBufferSize - _iPayloadWritePos < sizeof(iValue))
			Resize((int)(_iBufferSize * 1.5f));
	...
	inline CNetSendPacket& operator >> (BYTE& byValue)
	{
	...
	inline CNetSendPacket& operator >> (wchar_t& szValue)
	{
	...
	inline CNetSendPacket& operator >> (WORD& wValue)
	{
	...
	inline CNetSendPacket& operator >> (DWORD& dwValue)
	{
...
public:
	inline void CopyRecvBuf(CNetSendPacket* origin)
	{

...
void CGameServer::OnRecv(__int64 sessionID, CPacket* packet)
{
	...
	if (pPlayer->_state != PLAYER_STATE::ALIVE)
	{
		::printf("%s[%d] Player State is wrong!! (%d != %d)\n",
			__func__, __LINE__, PLAYER_STATE::ALIVE, pPlayer->_state);
	}
...
unsigned int __stdcall CGameServer::UpdateThread(void* arg)
{
	...
	while(!g_bShutdown)
	{
		pServer->PacketUpdate(num);
		if (pServer->SkipForFixedFrame()) continue;
...
{
	if ((GetTickCount64() - oldTick) < (1000 / dfFPS)) return true;
	oldTick += (1000 / dfFPS);
	return false;
}

inline void CGameServer::PacketUpdate(int threadNum)
{
	...
	for (int i = 0; i < dfLOGIC_PLAYER_NUM; i++)
	{
		if (_logicPlayers[threadNum][i]->_state == PLAYER_STATE::ALIVE ||
			_logicPlayers[threadNum][i]->_state == PLAYER_STATE::CONNECT)
		{
			...
			while (!pPlayer->_jobQ._queue.empty())
			{
				...
				else if (job->_type == JOB_TYPE::CONTENT)
				{
					...
					{
						::printf("[%s][%d] Player State is wrong! (%d != %d)",
							__func__, __LINE__, PLAYER_STATE::ALIVE, PLAYER_STATE::CONNECT);
					}
					HandleRecv(pPlayer, job->_packet);
					_pJobPool->Free(job);
				}
			}
		}
...
inline void CGameServer::HandleAccept(CPlayer* pPlayer)
{
...
inline void CGameServer::HandleRecv(CPlayer* pPlayer, CPacket* pPacket)
{

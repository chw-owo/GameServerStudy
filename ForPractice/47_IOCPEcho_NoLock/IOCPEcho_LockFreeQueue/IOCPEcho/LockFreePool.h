...
public:
	...
	LockFreePool(int blockNum, bool placementNew, Types... args);
...
template<class DATA>
template<typename... Types>
LockFreePool<DATA>::LockFreePool(int blockNum, bool placementNew, Types... args)
...
template<class DATA>
template<typename... Types>
DATA* LockFreePool<DATA>::Alloc(Types... args) // Pop
{
	if (_placementNew)
	{
...
template<class DATA>
bool LockFreePool<DATA>::Free(DATA* pData) // Push
{
	// For Protect ABA
	unsigned __int64 key = (unsigned __int64)InterlockedIncrement64(&_key);
	key <<= __USESIZE_64BIT__;

    ...
    void Enqueue(T data)
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                {
                    LeaveLog(1, size, newNode, next, 0, data);
                    InterlockedCompareExchange64(&_tail, newNode, tail);
                    LeaveLog(2, size, newNode, tail, 0, data);
                    break;
                }
            }
        }
...
private: // For Log
    ...
    private:
        ...
        void SetData(int idx, int threadID, int line, int size, int exchKey, __int64 exchAddress,
            int compKey, __int64 compAddress, int realKey, __int64 realAddress, T data)
        {
            _exchKey = exchKey;

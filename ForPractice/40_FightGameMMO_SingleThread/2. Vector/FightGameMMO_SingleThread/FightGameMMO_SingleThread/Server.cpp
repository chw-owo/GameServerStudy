...
Server::Server()
{
	...
	_PlayerMap.reserve(dfDEFAULT_PLAYERS_NUM);
	for (int i = 0; i < dfDEFAULT_PLAYERS_NUM; i++)
	{
		_PlayerMap.push_back(nullptr);
	}

	_SessionMap.reserve(dfDEFAULT_SESSIONS_NUM);
	for (int i = 0; i < dfDEFAULT_SESSIONS_NUM; i++)
	{
		_SessionMap.push_back(nullptr);
	}

	_emptySessionID.reserve(dfDEFAULT_SESSIONS_NUM);
	for (int i = dfDEFAULT_SESSIONS_NUM - 1; i >= 0 ; i--)
	{
...
Server::~Server()
{	
	vector<Session*>::iterator sessionIter = _SessionMap.begin();
	for (; sessionIter != _SessionMap.end(); sessionIter++)
	{
		if ((*sessionIter) == nullptr) continue;
		closesocket((*sessionIter)->_socket);
		_pSessionPool->Free(*sessionIter);
	}

	vector<Player*>::iterator playerIter = _PlayerMap.begin();
	for (; playerIter != _PlayerMap.end(); playerIter++)
	{
		if ((*playerIter) == nullptr) continue;
		_pPlayerPool->Free(*playerIter);
	}
...
{ 
	...
	_connected = dfDEFAULT_SESSIONS_NUM - _emptySessionID.size();
	...
	if (_connected > 0)
	{
		...
		for (; sessionIter != _SessionMap.end(); sessionIter++)
		{
			if ((*sessionIter) == nullptr)
				continue;

			FD_SET((*sessionIter)->_socket, &rset);
			if ((*sessionIter)->_sendBuf.GetUseSize() > 0)
				FD_SET((*sessionIter)->_socket, &wset);

			_sessionArray[idx] = *sessionIter;
			idx++;
...
void Server::ContentUpdate()
{
	...
	for (; playerIter != _PlayerMap.end(); playerIter++)
	{
		...
		if (timeGetTime() - (*playerIter)->_pSession->_lastRecvTime > dfNETWORK_PACKET_RECV_TIMEOUT)
		{
			_timeoutCnt++;
...
void Server::AcceptProc()
{
	...
	{
		::printf("Error! Func %s Line %d: too much connect\n", __func__, __LINE__);
		LOG(L"ERROR", SystemLog::ERROR_LEVEL,
			L"%s[%d]: new Error, %d\n",

...
bool CNetServer::MoveGroup(unsigned __int64 sessionID, CGroup* pGroup)
{
	...
	CGroup* pCurGroup = pSession->_pGroup;
	InterlockedExchangePointer((PVOID*)&pSession->_pGroup, pGroup);

	if(pCurGroup != nullptr)
	{
		pCurGroup->_leaveSessions.Enqueue(pSession);
	}
...
void CNetServer::DecrementUseCount(CSession* pSession, int line)
{
	...
	if (ret == 0)
	{
		if (InterlockedCompareExchange(&pSession->_validFlag._flag, _releaseFlag._flag, 0) == 0)
		{
			...
			{
				pSession->_pGroup->_releaseSessions.Enqueue(pSession);
			}

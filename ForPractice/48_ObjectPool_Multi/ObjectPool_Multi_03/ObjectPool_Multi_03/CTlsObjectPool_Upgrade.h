...
public:
	CTlsObjectPool_Upgrade(int blockNum, bool placementNew);
...
template<class T>
CTlsObjectPool_Upgrade<T>::CTlsObjectPool_Upgrade(int blockNum, bool placementNew)
...
template<class T>
template<typename... Types>
inline int CTlsObjectPool_Upgrade<T>::RegisterThread(bool alloc, Types... args)
{
	int threadIdx = (int)TlsGetValue(_tlsIdx);

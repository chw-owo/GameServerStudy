...
public:
	...
	{
	public:
		CPool(CTlsObjectPool<T>* mainPool, bool placementNew);
		~CPool();
...
template<class T>
inline CTlsObjectPool<T>::CPool::CPool(CTlsObjectPool<T>* mainPool, bool placementNew) : _mainPool(mainPool), _placementNew(placementNew)
{
	_return = new stBucket;
}

template<class T>
inline CTlsObjectPool<T>::CPool::~CPool()
{
	if (_placementNew)
	{
...
template<class T>
inline void CTlsObjectPool<T>::CPool::ReturnBucket()
{
...
template<class T>
template<typename ...Types>
inline void CTlsObjectPool<T>::CPool::GetBucket(Types ...args)
{
	for (;;)
	{
		__int64 pCur = _mainPool->_buckets;
...
template<class T>
template<typename ...Types>
inline void CTlsObjectPool<T>::CPool::CreateBucket(Types ...args)
{
...
template<class T>
template<typename ...Types>
inline T* CTlsObjectPool<T>::CPool::Alloc(Types ...args)
{
...
template<class T>
inline void CTlsObjectPool<T>::CPool::Free(T* data)
{
...
template<class T>
template<typename ...Types>
inline CTlsObjectPool<T>::CTlsObjectPool(int blockNum, bool placementNew, Types ...args)
{
	_tlsIdx = TlsAlloc();
...
template<class T>
inline CTlsObjectPool<T>::~CTlsObjectPool()
{
...
template<class T>
template<typename ...Types>
inline T* CTlsObjectPool<T>::Alloc(Types ...args)
{
...
template<class T>
template<typename ...Types>
inline void CTlsObjectPool<T>::Initialize(Types ...args)
{
	CPool* pool = (CPool*)TlsGetValue(_tlsIdx);
	if (pool == nullptr)
	{
...
template<class T>
inline void CTlsObjectPool<T>::Free(T* data)
{
	CPool* pool = (CPool*)TlsGetValue(_tlsIdx);
	if (pool == nullptr)
	{
